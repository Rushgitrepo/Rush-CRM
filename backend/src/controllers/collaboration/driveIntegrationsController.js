...
const oauthCallback = async (req, res, next) => {
  try {
    ...
    res.redirect(`${process.env.APP_URL || 'http://localhost:5173'}/collaboration/drive?connected=google_drive&driveId=${finalDriveId}`);
  } catch (error) {
    console.error('OAuth Callback Error:', error);
    res.redirect(`${process.env.APP_URL || 'http://localhost:5173'}/collaboration/drive?error=${encodeURIComponent(error.message)}`);
  }
};

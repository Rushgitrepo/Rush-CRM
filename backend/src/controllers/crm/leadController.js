  ...
  try {
    ...
    if (workspaceId) {
      ...
      query += ` AND (
        ...
        ${hasUniboxAccess ? " OR EXISTS (SELECT 1 FROM unibox_emails ue WHERE ue.sender_email = l.email AND ue.org_id = l.org_id) OR (l.source = 'Instantly' AND EXISTS (SELECT 1 FROM instantly_integrations ii WHERE ii.org_id = l.org_id AND ii.auto_add_leads = true))" : ""}
      )`;

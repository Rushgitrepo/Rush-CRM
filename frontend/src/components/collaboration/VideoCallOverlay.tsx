...
function RemotePeerVideo({ peer, fullScreen }: { peer: any, fullScreen?: boolean }) {
  ...
  const initials = peer.name
    ? peer.name.split(' ').map((w: string) => w[0]).join('').toUpperCase().slice(0, 2)
    : '??';

  return (
    <div className={cn(
      "relative overflow-hidden group transition-all duration-500",
      fullScreen ? "w-full h-full rounded-none" : "aspect-video bg-slate-900 rounded-2xl border border-white/5 shadow-2xl"
    )}>
      ...
      <video
        ...
        className={cn(
          ...
          ((peer.isVideoOff && !peer.isScreenSharing) || !peer.stream) ? "opacity-0" : "opacity-100"
        )}
      />
      ...
      {((peer.isVideoOff && !peer.isScreenSharing) || !peer.stream) && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-slate-950 z-0">
          <div className={cn(
            ...
            {peer.avatar ? (
              <img src={getAvatarUrl(peer.avatar)} alt={peer.name} className="w-full h-full object-cover" />
            ) : (
              <span className={cn("font-bold text-white/50", fullScreen ? "text-5xl" : "text-3xl")}>{initials}</span>
            )}
          </div>
          <span className="text-white font-medium text-lg tracking-tight">{peer.name}</span>
          <div className="flex items-center gap-2 px-3 py-1 bg-white/5 rounded-full backdrop-blur-md">
             <div className="w-1.5 h-1.5 rounded-full bg-red-500" />
             <span className="text-[10px] text-white/40 uppercase font-bold tracking-widest">Camera Off</span>
          </div>
        </div>
      )}
  ...
  return (
    <div className="w-full h-full bg-zinc-950 flex flex-col items-center justify-center gap-12 py-24 px-8 animate-in fade-in duration-700 relative overflow-hidden">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden">
        ...
        {avatarUrl ? (
          <img src={avatarUrl} className="w-full h-full object-cover blur-[120px] scale-150 opacity-40" alt="" />
        ) : (
      ...
      <div className="relative z-10 text-center flex flex-col items-center">
        <div className="flex items-center gap-3 mb-6 bg-white/5 px-4 py-2 rounded-full border border-white/5 backdrop-blur-md">
          ...
          <p className={cn("font-bold tracking-[0.2em] uppercase text-[12px] text-white", status && "text-red-500")}>
            {status || (isRoomJoin ? 'Connecting to meeting...' : (callType === 'video' ? 'Video Calling...' : 'Calling...'))}
          </p>
        </div>
      ...
      <div className="relative z-10">
        <div className="relative group">
           <div className={cn("absolute inset-0 rounded-full scale-150 blur-2xl animate-pulse duration-[3000ms]", status ? "bg-red-500/20" : "bg-indigo-500/20")} />
           <div className={cn("absolute inset-0 rounded-full scale-125 animate-ping duration-[2000ms]", status ? "bg-red-500/10" : "bg-indigo-500/10")} />
           <Avatar className="h-64 w-64 border-8 border-white/10 shadow-[0_0_100px_rgba(0,0,0,0.5)] relative z-10 transition-transform duration-700 group-hover:scale-105">
             {avatarUrl ? <AvatarImage src={avatarUrl} /> : null}
             <AvatarFallback className="bg-zinc-800 text-7xl font-bold text-zinc-500">
               {isRoomJoin ? <Monitor className="w-24 h-24" /> : peer?.name?.charAt(0)}
             </AvatarFallback>
           </Avatar>
        </div>
      </div>
...
function AudioCallView({ peer, duration, isMuted, toggleMute, endCall, renderInvitePopover }: any) {
  const avatarUrl = getAvatarUrl(peer.avatar);
...
export default function VideoCallOverlay() {
  ...
  const inviteableUsers = useMemo(() => {
    ...
    return allUsers.filter(u => u.id !== currentUser?.id && !peerIds.has(u.id));
    ...
    <Popover open={showInvitePopover} onOpenChange={setShowInvitePopover}>
      ...
      >
        <Command className="bg-transparent">
          <CommandInput placeholder="Search people..." className="text-white text-xs h-10" />
          <CommandList className="max-h-60">
            <CommandEmpty className="p-4 text-center text-xs text-white/40 italic">No users found</CommandEmpty>
            <CommandGroup className="px-2 pb-2">
              {inviteableUsers.map(u => (
                <CommandItem 
                  ...
                  value={u.full_name}
                  onSelect={() => { 
                    console.log(`[VideoCall] Inviting user: ${u.full_name} (${u.id})`);
                    inviteToCall(u.id); 
                ...
                >
                   <Avatar className="h-7 w-7">
                     ...
                     <AvatarFallback className="text-[10px] bg-zinc-800">{u.full_name?.charAt(0)}</AvatarFallback>
                   </Avatar>
                   <div className="flex flex-col min-w-0">
                     <span className="text-[11px] font-medium text-white truncate">{u.full_name}</span>
          ...
          <div className="flex flex-col items-center text-center">
            <div className="relative mb-8">
              <div className="w-28 h-28 rounded-full bg-indigo-500/10 flex items-center justify-center animate-pulse">
                <Avatar className="h-24 w-24 border-4 border-zinc-900 shadow-2xl overflow-hidden">
                   <AvatarImage src={getAvatarUrl(firstPeer?.avatar)} />
                   <AvatarFallback className="bg-indigo-600 text-white text-3xl font-bold">{firstPeer?.name?.charAt(0)}</AvatarFallback>
                </Avatar>
              </div>
              <div className="absolute -bottom-1 -right-1 w-10 h-10 rounded-full bg-indigo-600 flex items-center justify-center border-4 border-zinc-900 text-white">
                {callType === 'video' ? <Video className="w-5 h-5 font-bold" /> : <Phone className="w-5 h-5 font-bold" />}
              </div>
            </div>
            <h2 className="text-2xl font-bold text-white mb-2">{firstPeer?.name}</h2>
            <p className="text-zinc-500 tracking-widest uppercase text-[10px] font-bold mb-10">{callType === 'video' ? 'Video Call...' : 'Audio Call...'}</p>
            <div className="flex gap-4 w-full">
              <button onClick={rejectCall} className="flex-1 h-16 rounded-[24px] bg-red-500 hover:bg-red-600 flex items-center justify-center shadow-lg transition-transform hover:scale-105 active:scale-95"><PhoneOff className="w-8 h-8 text-white" /></button>
              <button onClick={acceptCall} className="flex-1 h-16 rounded-[24px] bg-emerald-500 hover:bg-emerald-600 flex items-center justify-center shadow-lg transition-transform hover:scale-105 active:scale-95"><Phone className="w-8 h-8 text-white" /></button>
            </div>
          </div>
        </div>
      </div>,
  ...
  const renderCallContent = () => {
    ...
    if (callStatus || callState === 'outgoing' || callState === 'connecting') {
      return <OutgoingCallView peer={firstPeer} callType={callType} status={callStatus} endCall={endCall} />;
    }

    // Audio View (Only for 1-on-1 private calls)
    if (callType === 'audio' && peerList.length === 1 && !isGroupCall) {
      return <AudioCallView peer={peerList[0]} duration={callDuration} isMuted={isMuted} toggleMute={toggleMute} endCall={endCall} renderInvitePopover={renderInvitePopover} />;
    }
    ...
    return (
      <div className={cn(
        ...
        <div className="flex-1 relative overflow-hidden flex flex-col">
          ...
          {anyScreenSharing ? (
            <div className="flex-1 bg-black relative flex items-center justify-center">
              {isScreenSharing ? (
                <video ref={screenVideoRef} autoPlay playsInline className="max-w-full max-h-full object-contain" />
              ) : (
              ...
              <div className="absolute top-4 left-4 flex items-center gap-2 bg-indigo-600 px-3 py-1 rounded-full text-[10px] font-bold text-white shadow-xl animate-pulse z-50">
                ...
                {isScreenSharing ? 'YOU ARE SCREEN SHARING' : `${activeScreenSharePeer?.name?.toUpperCase()} IS SHARING`}
              </div>
              ...
              <div className="absolute bottom-4 left-4 right-4 flex justify-center gap-2 overflow-x-auto p-2 no-scrollbar z-50">
                 <div className="w-32 aspect-video rounded-xl bg-zinc-800 border border-white/20 overflow-hidden shrink-0 relative">
                    {isVideoOff ? <div className="w-full h-full flex items-center justify-center text-[10px] text-white/20 font-bold uppercase">You</div> : <video ref={localVideoRef} autoPlay muted playsInline className="w-full h-full object-cover scale-x-[-1]" />}
                 </div>
                 {peerList.map(p => (
                   /* Don't show the presenter in the strip if they are the main view (optional, but Zoom usually keeps them in the strip too) */
                   <div key={p.userId} className="w-32 aspect-video rounded-xl bg-zinc-800 border border-white/20 overflow-hidden shrink-0 relative">
            ...
            <div className={cn(
              ...
              isGroupCall ? "bg-zinc-950" : "px-4"
               ...
               <div className={cn(
                 ...
                 isGroupCall 
                   ? (peerList.length === 0 ? "grid-cols-1" : peerList.length === 1 ? "grid-cols-1 lg:grid-cols-2" : "grid-cols-1 md:grid-cols-2 lg:grid-cols-3")
                     ...
                     {isVideoOff || !localStream ? (
                       <div className="w-full h-full flex flex-col items-center justify-center gap-4">
                         <Avatar className="h-20 w-20 border-2 border-white/10">
                            <AvatarFallback className="bg-zinc-800 text-2xl font-bold text-zinc-500">{currentUser?.full_name?.charAt(0)}</AvatarFallback>
                         </Avatar>
                         <span className="text-[11px] text-white/20 font-bold uppercase tracking-widest">You</span>
                       </div>
                     ) : (
        ...
        <div className={cn(
          ...
          isGroupCall ? "bg-zinc-900/50 backdrop-blur-3xl border-t border-white/5 px-6 py-4" : "h-0"
        )}>
          <div className={cn(
            "flex items-center justify-between max-w-5xl mx-auto",
            !isGroupCall && "absolute bottom-12 left-1/2 -translate-x-1/2 flex items-center gap-3 px-4 py-3 bg-white/5 backdrop-blur-3xl rounded-[32px] border border-white/10 z-30 transition-all",
            !isGroupCall && !showControls && "opacity-0 translate-y-10 scale-90 pointer-events-none"
          )}>
            {/* Left Info (Zoom style - Meeting ID/Time) */}
            {isGroupCall && (
              <div className="hidden lg:flex items-center gap-3 text-white/40">
                <div className="flex flex-col">
                  <span className="text-[10px] font-bold uppercase tracking-tighter text-indigo-400">Security</span>
                  <span className="text-[11px] font-medium text-emerald-500">Encrypted</span>
                </div>
              </div>
            )}
            ...
            <div className="flex items-center gap-3 mx-auto">
              ...
              <button onClick={toggleMute} className={cn(
                ...
                isMuted ? "bg-red-500 shadow-[0_0_20px_rgba(239,68,68,0.3)]" : "bg-white/10 hover:bg-white/20"
              )}>
                {isMuted ? <MicOff className="w-5 h-5 text-white" /> : <Mic className="w-5 h-5 text-white" />}
              </button>
              <button onClick={toggleVideo} className={cn(
                "p-3.5 rounded-2xl transition-all hover:scale-105 active:scale-95",
                isVideoOff ? "bg-red-500 shadow-[0_0_20_rgba(239,68,68,0.3)]" : "bg-white/10 hover:bg-white/20"
              )}>
              ...
              <button onClick={toggleScreenShare} className={cn(
                 "p-3.5 rounded-2xl transition-all hover:scale-105 active:scale-95 flex items-center justify-center",
                 isScreenSharing ? "bg-emerald-500 text-white shadow-[0_0_20_rgba(16,185,129,0.3)]" : "bg-white/10 text-white hover:bg-white/20"
              )}>
              ...
              <button onClick={() => setShowReactionPicker(!showReactionPicker)} className={cn("p-3.5 rounded-2xl bg-white/10 text-white hover:bg-white/20 transition-all hover:scale-105 active:scale-95", showReactionPicker && "bg-white text-zinc-950")}>
              ...
              {/* End Call */}
              <button onClick={endCall} className="px-6 h-12 rounded-2xl bg-red-600 hover:bg-red-500 text-white text-xs font-bold uppercase tracking-widest shadow-xl transition-all active:scale-95 hover:scale-105">
                Leave Call
              </button>
            </div>
  ...
  const content = (
    <div className={cn("w-full h-full relative", isGroupCall ? "bg-zinc-950" : "")}>
      {/* Dynamic Header */}
      <div className={cn(
        ...
        isGroupCall ? "top-8" : "top-10"
         ...
         <div className="flex items-center gap-3">
           <div className="flex items-center gap-2 bg-black/40 px-3 py-1.5 rounded-full backdrop-blur-xl border border-white/5">
             ...
             <span className="text-white text-[11px] font-bold tracking-tight">{formatDuration(callDuration)}</span>
           </div>
           {isGroupCall && (
             <div className="flex -space-x-2">
                <Avatar className="h-6 w-6 border-2 border-zinc-950">
                  <AvatarFallback className="bg-zinc-800 text-[8px] font-bold text-white">{currentUser?.full_name?.charAt(0)}</AvatarFallback>
                </Avatar>
                {peerList.map(p => (
                  <Avatar key={p.userId} className="h-6 w-6 border-2 border-zinc-950">
                     <AvatarImage src={getAvatarUrl(p.avatar)} />
                     <AvatarFallback className="bg-zinc-700 text-[8px] font-bold text-white">{p.name?.charAt(0)}</AvatarFallback>
                  </Avatar>
                ))}
             </div>
           )}
         ...
         {!isGroupCall && peerList.length > 1 && (
           <div className="text-[10px] text-white font-bold bg-white/5 px-2 py-1 rounded mt-1">{peerList.length + 1} ON CALL</div>
         )}
      </div>
      ...
      <div className="absolute top-1/2 left-0 right-0 pointer-events-none z-[100] flex flex-col items-center">
        {reactions.map((r) => (
          <div key={r.id} className="animate-reaction flex flex-col items-center mb-4">
            <span className="text-6xl drop-shadow-[0_10px_20px_rgba(0,0,0,0.6)]">{r.emoji}</span>
            <span className="text-[10px] text-white/50 font-bold uppercase mt-2 tracking-widest">{r.userName}</span>
          </div>
        ))}
      </div>
    </div>

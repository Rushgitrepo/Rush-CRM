  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const result = response || { hasPermission: false, isOwner: false, hasFullAccess: false, isRestricted: false, assignedFolderCount: 0 };
        console.log('Final result:', result);
        return result;
      } catch (error) {
        console.error('Unibox permission check failed:', error);
        return { hasPermission: false, isOwner: false, hasFullAccess: false, isRestricted: false, assignedFolderCount: 0 };
      }
    },
    enabled: !!user,

...
export default function MicrosoftCallbackPage() {
  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      if (stateParam) {
        try {
          ...
          if (stateData.provider === "onedrive") {
            provider = "onedrive";
          } else if (stateData.provider === "outlook-mail") {
      ...
      const isMail = provider === "outlook-mail";
      const isOneDrive = provider === "onedrive";
      const redirectPath = isMail
        ? "/collaboration/mail"
        : isOneDrive
        ? "/collaboration/drive"
      ...
      if (error) {
        setStatus("error");
        setMessage(`Microsoft authorization failed: ${errorDescription || error}`);
      ...
      try {
        let endpoint = "/integrations/microsoft/exchange-code";
        if (isMail) {
          endpoint = "/integrations/outlook/exchange-code";
        } else if (isOneDrive) {
          endpoint = "/integrations/onedrive/exchange-code";
        }

        const data = await api.post<any>(endpoint, { code });

        setStatus("success");
        const successMsg = isMail
          ? "Outlook connected! Syncing your emails..."
          : isOneDrive
          ? "OneDrive connected successfully!"
          : "Microsoft Calendar connected successfully!";
        setMessage(successMsg);
        toast({
          title: isMail ? "Outlook Connected" : isOneDrive ? "OneDrive Connected" : "Calendar Connected",
